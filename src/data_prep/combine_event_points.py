...
def main():
    ...
    for past_event_name in past_event_names:
        driver_points_df_slim = pd.read_csv(f".data/driver_points_{year}_{past_event_name}.csv")
        if driver_points_df_slim.empty:
            continue
        driver_points_df = pd.concat([driver_points_df, driver_points_df_slim], ignore_index=True)
        constructor_points_df_slim = pd.read_csv(f".data/constructor_points_{year}_{past_event_name}.csv")
    ...
    driver_points_df.to_csv(f"data/driver_points_{year}_current.csv", index=False)
    constructor_points_df.to_csv(f"data/constructor_points_{year}_current.csv", index=False)
